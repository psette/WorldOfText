...
public class FinalistsAndEliminatedTeams implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -159582676398753378L;
	...
	// a list of the teams who have been eliminated from the game (will not be
	// in final jeopardy)
	...
	// note that the return type is and ArrayList<TeamData> NOT
	// ArrayList<TeamGUIComponents>
	// this is because the WinnersGUI only needs to access the team name, it
	// doesn't need the GUI information
	// yay abstraction!!
	public ArrayList<TeamData> getWinners() {
		...
		if (finalists.size() != 0) {
			...
			// the team at the end of the list must have the highest score and
			...
			if (finalists.size() > 1) {
				...
				for (int i = finalists.size() - 2; i > -1; i--) {
					...
					// if this team has the same score as the max, it must also
					// be a winner
					if (finalists.get(i).getPoints() == max)
