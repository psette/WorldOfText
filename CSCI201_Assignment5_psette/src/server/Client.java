...
public class Client extends Thread {
	...
	public Client(String port, String IPAddress, String teamName, StartWindowGUI startWindowGUI, Boolean isHost,
			User loggedInUser) {
		...
		try {
			...
			// before we enter the run method, we want to send our team name and
			...
			try {
				// read our assigned team index and how many teams are needed
				// right now
	...
	public void run() {
		...
		try {
			...
			// repopulate game data GUI objects since all GUI components are
			// transient in GameData and subsequent classes
			gameData.populate();
			startWindowGUI.dispose();
			mainGUI = new MainGUINetworked(gameData, this, loggedInUser);
			mainGUI.setVisible(true);

			if (gameData.getCurrentTeam().getTeamIndex() != teamIndex)
