...
public class QuestionAnsweredAction extends Action {
	...
	public void executeAction(MainGUINetworked mainGUI, ServerGameData gameData, Message message, Client client) {
		...
		if (answer.equals("Too Late!")) {
			currentTeam.deductPoints(currentQuestion.getPointValue());
			mainGUI.addUpdate(currentTeam.getTeamName() + " did not answer in time! $" + currentQuestion.getPointValue()
					+ " will be deducted from their total. ");
			// check if all teams have gotten a chance to answer
			questionDone();
			currentQuestion.setWaitBuzz(true);
			currentQuestion.answerField.setText("Please enter an answer.");
		} else if (answer.equals("No one clicked!")) {
			mainGUI.addUpdate("No one buzzed in! Returning to main screen!");
			questionIsDone();
	...
	private void validFormat() {
		...
		if (correct) {
			// add points, add update on Game Progress, set the next turn to the
			// current team
	...
	private void checkReadyForFJ() {
		// check with the game data if we are ready for FJ, if so have the
		// mainGUI set it up
		...
		// if not ready, move on to another question
			...
			// add update to Game Progress, determine whether the game board
			// buttons should be enabled or disabled, and change the main panel
	...
	private void questionIsDone() {
		gameData.setNextTurn(gameData.nextTurn(currentQuestion.getOriginalTeam()));
		mainGUI.addUpdate("The correct answer was: " + currentQuestion.getAnswer());
		checkReadyForFJ();
	}

	private void questionDone() {
		// check the current question to see if all teams have had a chance, and
		// if so
		if (currentQuestion.questionDone()) {
			// set the next turn to be the team in clockwise order from original
			// team, add an update, and see whether it is time for FJ
			gameData.setNextTurn(gameData.nextTurn(currentQuestion.getOriginalTeam()));
			mainGUI.addUpdate(
					"No one answered the question correctly. The correct answer was: " + currentQuestion.getAnswer());
			checkReadyForFJ();
		}
		// if question has not been attempted by everyone, go to a buzz in
		// period
		else {
			currentQuestion.setBuzzInPeriod();
			currentQuestion.startTimer(19);
			mainGUI.addUpdate("Buzz in to answer!");
		}
	}

	private void invalidFormat() {
		// if the current team has already had a second chance, deduct points,
		// sent an update to the Game Progress
		...
		// if they have not had a second chance yet, stay on the same team, just
		// send an update to the Game Progress
		else {
			// this method changes the annnouncements label on the question
			// panel to say they had illformatted answer
