...
public class StartWindowGUI extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6740759991952725246L;
	// text fields
	public JTextField port;
	public JTextField IPAddress;
	// radio buttons
	public JRadioButton notNetworked;
	public JRadioButton hostGame;
	public JRadioButton joinGame;
	// labels
	...
	public List<JTextField> teamNameTextFields;
	...
	private void createGUI() {
		...
		AppearanceSettings.setFont(AppearanceConstants.fontSmall, teamNameTextFields.get(0), teamNameTextFields.get(1),
				...
				clearDataButton, logoutButton, startGameButton, hostGame, notNetworked, joinGame, IPAddress, port,
				messageLabel);
		...
		AppearanceSettings.setForeground(Color.lightGray, exitButton, logoutButton, clearDataButton, startGameButton,
				...
				fileChooserButton, fileNameLabel, slider, messageLabel);
		...
		AppearanceSettings.setTextAlignment(messageLabel, teamNameLabels.get(0), teamNameLabels.get(1),
				teamNameLabels.get(2), teamNameLabels.get(3), fileNameLabel);
	...
	private void createMainPanel() {
		...
		JLabel explainContentLabel = new JLabel(
				"Choose whether you are joining or hosting a game or playing not-networked.");
		...
		AppearanceSettings.setBackground(AppearanceConstants.lightBlue, radioButtonPanel, welcomeLabel,
				explainContentLabel, welcomePanel, titlePanel);
		...
		AppearanceSettings.setBackground(AppearanceConstants.darkBlue, networkingPanel, chooseGameFileLabel,
				numberOfTeamsLabel, numberOfTeamsPanel, fileChooserPanel, teamsAndFilePanel, buttonPanel,
				teamNamesPanel, teamLabelsPanel1, teamLabelsPanel2, teamTextFieldsPanel1, teamTextFieldsPanel2,
				messagePanel, buttonPanel, messageLabel);
		...
		AppearanceSettings.setBoxLayout(BoxLayout.LINE_AXIS, radioButtonPanel, buttonPanel, fileChooserPanel,
	...
	public void wrapper(File file) {
		setHaveValidFile(file, serverGameData);
	}
	...
	private void setFileNameLabel(GameData gameData, String fileName) {
		if (gameData.getGameFile().getNumberOfRatings() == -1)
			fileNameLabel.setText(fileName + "    no rating");

		else
			fileNameLabel.setText(fileName + "    average rating: " + gameData.getGameFile().getAverageRating() + "/5");
	}

	// returns whether the start button can be enabled depending on the radio
	// button selection
	private boolean ready() {
		...
		if (notNetworked.isSelected()) {
			...
			for (int i = 0; i < slider.getValue(); i++) {
				if (teamNameTextFields.get(i).getText().isEmpty())
					return false;
			}
			return true;
		}
	...
	public void gameCancelled() {
		...
		AppearanceSettings.setEnabled(true, startGameButton, port, IPAddress, teamNameTextFields.get(0),
				clearDataButton, notNetworked, hostGame, joinGame, fileChooserButton);
	}
	...
	private void addListeners() {
		...
		fileChooserButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (file != null) {
					...
					if (notNetworked.isSelected())
						setHaveValidFile(file, gameData);
					else
						setHaveValidFile(file, serverGameData);
				}
			}
		...
		startGameButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (notNetworked.isSelected()) {
					...
					// disable components
					AppearanceSettings.setEnabled(false, startGameButton, quickPlay, clearDataButton, joinGame, port,
							IPAddress, teamNameTextFields.get(0), hostGame, notNetworked, fileChooserButton);
					...
					if (hostGame.isSelected()) {
						...
						client = new Client(port.getText(), "localhost", teamNameTextFields.get(0).getText(),
								StartWindowGUI.this, true, loggedInUser);
					} else {
						client = new Client(port.getText(), IPAddress.getText(), teamNameTextFields.get(0).getText(),
								StartWindowGUI.this, false, loggedInUser);
					}
		...
		notNetworked.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (haveValidFile)
					setFileNameLabel(gameData, gameData.getFileName());
				else
					fileNameLabel.setText("");
			}
	...
	private void notNetworkedSelected(Boolean startGame) {
		AppearanceSettings.setVisible(false, port, IPAddress);
		AppearanceSettings.setVisible(true, slider, numberOfTeamsLabel, fileNameLabel, chooseGameFileLabel,
				fileChooserButton, quickPlay);
	...
	private void netWorkingSelected() {
		...
		if (hostGame.isSelected()) {
			...
			AppearanceSettings.setVisible(true, slider, port, quickPlay, numberOfTeamsLabel, fileNameLabel,
					chooseGameFileLabel, fileChooserButton);

			if (haveValidNetworkedFile)
				setFileNameLabel(serverGameData, serverGameData.getFileName());
			else
				fileNameLabel.setText("");
			startGameButton.setText("Start Game");
			slider.setMinimum(2);
			...
			AppearanceSettings.setVisible(false, slider, quickPlay, numberOfTeamsLabel, fileNameLabel,
					chooseGameFileLabel, fileChooserButton);
