...
public class FactoryNode extends FactoryObject {
	...
	public void draw(Graphics g, Point mouseLocation) {
		...
		if (mFObject == null) {
			g.setColor(Color.WHITE); // draw a border, makes it look like a
										// tiled grid in the factory
		...
		while (!openList.isEmpty()) {
			...
			for (FactoryNode neighbor : current.fNode.mNeighbors) {
				if (neighbor.getObject() != null) {
					if (neighbor != mDestinationNode)
						continue;
				}
				if (containsNode(closedList, neighbor) != null)
					continue;
				int temp_gScore = current.gScore + 1;// nodes always have
														// distance 1 in our
														// case
