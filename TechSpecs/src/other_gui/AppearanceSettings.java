...
public class AppearanceSettings {
    ...
    //underline font of supplied JComponents
    @SuppressWarnings("unchecked")
    @SafeVarargs
    public static <T extends JComponent>void setFontUnderlined(Font font, T ... components ){
        for (T component : components) {
    ...
    //bold font of supplied JComponents
    @SafeVarargs
    public static <T extends JComponent>void setFontBold(T ... components ){
        for (T component : components) {
            String name = component.getFont().getFontName();
            int size = component.getFont().getSize();
            if (component.getFont().isBold() && component.getFont().isItalic()) { //unbold but keep italic
                Font font = new Font(name, Font.ITALIC, size);
                component.setFont(font);
            } else if (component.getFont().isBold() && !component.getFont().isItalic()) { //unbold and not italic
                Font font = new Font(name, Font.PLAIN, size);
                component.setFont(font);
            } else if (!component.getFont().isBold() && component.getFont().isItalic()) { //bold and keep italic
                Font font = new Font(name, Font.BOLD | Font.ITALIC, size);
                component.setFont(font);
            } else { //bold and not italic
    ...
    //underline font of supplied JComponents
    @SafeVarargs
    public static <T extends JComponent>void setFontItalic(T ... components ){
        for (T component : components) {
            String name = component.getFont().getFontName();
            int size = component.getFont().getSize();
            if (component.getFont().isBold() && component.getFont().isItalic()) { // unitalicize but keep bold
                Font font = new Font(name, Font.BOLD, size);
                component.setFont(font);
            } else if (component.getFont().isBold() && !component.getFont().isItalic()) { // italicize and keep bold
                Font font = new Font(name, Font.BOLD | Font.ITALIC, size);
                component.setFont(font);
            } else if (!component.getFont().isBold() && component.getFont().isItalic()) { // unitalicize and keep unbold
                Font font = new Font(name, Font.PLAIN, size);
                component.setFont(font);
            } else { // italicize and keep unbold
                Font font = new Font(name, Font.ITALIC, size);
                component.setFont(font);
            }
        }
